...
import { useSetores } from '@/hooks/useSetores'; // 1. Importar o hook de setores
...
  const { setores } = useSetores(); // 2. Obter a lista de setores
  const [loading, setLoading] = useState(false);
  const [dadosHistoricos, setDadosHistoricos] = useState<RegulacaoHistorico[]>([]);

  // 3. Criar um mapa para converter nome do setor para sigla
  const mapaNomesParaSiglas = useMemo(() => {
    if (!setores) return new Map();
    return new Map(setores.map(s => [s.nomeSetor, s.siglaSetor]));
  }, [setores]);
...
    const dataFimFormatada = isValid(new Date(dataFim)) ? format(new Date(dataFim), 'dd/MM HH:mm') : 'N/A';
...
            
            // 4. Usar o mapa para obter as siglas
            const siglaOrigem = mapaNomesParaSiglas.get(reg.setorOrigemNome) || reg.setorOrigemNome;
            const siglaDestino = mapaNomesParaSiglas.get(reg.setorDestinoNome) || reg.setorDestinoNome;

            // Simplificação para mostrar sempre o destino final, que é mais claro no histórico
            const destinoFinal = `${siglaDestino} ${reg.leitoDestinoCodigo}`;

            texto += `- *${reg.pacienteNome}* (${siglaOrigem} - ${reg.leitoOrigemCodigo} -> ${destinoFinal})\n`;
            texto += `  _Iniciada em: ${dataCriacaoFormatada}_\n`;

            // Itera sobre os eventos para mostrar a linha do tempo de alterações
...
                    const motivo = evento.detalhes.split('Motivo: ')[1] || 'N/A';
                    // Nota: O log de alteração não armazena a sigla, então mantemos como está
